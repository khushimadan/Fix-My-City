...
class _ManageComplaintScreenState extends State<ManageComplaintScreen> {
  ...
  Future<void> fetchWorkers() async {
    try {
      ...
      List<DocumentSnapshot> availableWorkers = snapshot.docs;

      // Sort by number of assigned complaints (least first)
      availableWorkers.sort((a, b) {
        int aCount = (a['assignedComplaintNo'] as List?)?.length ?? 0;
        int bCount = (b['assignedComplaintNo'] as List?)?.length ?? 0;
        return aCount.compareTo(bCount);
      });

      if (availableWorkers.isNotEmpty && selectedWorkers.isEmpty) {
        // Auto-select top 1 worker only if none already selected
        setState(() {
          selectedWorkers.add(availableWorkers.first);
          // Remove the auto-selected one from the remaining list
          workers = availableWorkers.skip(1).toList();
          workersAssigned = true;
        });

        await assignWorkers(); // Immediately assign the auto-selected one
      } else {
        setState(() {
      ...
      body: complaintDocId == null
        ...
        child: ListView(
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              "Critical: ${widget.data['isCritical']==true ? 'Yes' : 'No'}",
              style: const TextStyle(fontSize: 16),
            ),
